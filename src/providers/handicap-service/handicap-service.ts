...
import {Events, Content} from 'ionic-angular/index';
...
// import {PlainScoreCard, PlayerRoundScores, PlayerScore, ScorecardList} from '../../data/scorecard';
...
import { PremiumFeatureBundle } from '../../data/premium-subscription';
import {HandicapCalculation, HandicapGameRound, ClubHandicap,
    PlainScoreCard, PlayerRoundScores, PlayerScore, ScorecardList} from '../../data/mygolf.data'
/*
...
export class HandicapService {
    ...
    public getHandicapHistory(playerId?: number): Observable<Array<HandicapCalculation>> {
        let hdrs = {};
        ...
        let request = new RemoteRequest(global.ServerUrls.getPlayerHcpList,
            RequestMethod.Get, ContentType.URL_ENCODED_FORM_DATA, null, hdrs);
            return this.http.execute(request)
            .map((resp: Response) => {
                console.log(resp)
                let handicapCalc: Array<HandicapCalculation> = resp.json();
                handicapCalc.forEach((h: HandicapCalculation) => {
                    // 2020-01-30T14:45:05+08:00[Asia/Kuala_Lumpur]
                    // YYYY-MM-DDTHH:mm:ssZ
                    let calcTime = String(h.calculatedOn).split("[");
                    // console.log("[calctime] calcTime ", calcTime);
                    if(calcTime[1]) {
                        h.calculatedOn = moment(calcTime[0],"YYYY-MM-DDTHH:mm:ssZ").toDate();
                        h.gameRounds.forEach((gr: HandicapGameRound) => {
                            let rDate = String(gr.roundDate).split("[");
                            // console.log("[calctime] calcTime ", calcTime);
                            gr.roundDate = moment(rDate[0],"YYYY-MM-DDTHH:mm:ssZ").toDate();
                            
                    })
                }
                })
                handicapCalc = handicapCalc.sort((a,b)=>{
    ...
    public getClubHandicap(playerId?: number): Observable<Array<ClubHandicap>> {
        // hcpCalculation
        let request = new RemoteRequest(global.ServerUrls.getPlayerClubHcp,
            RequestMethod.Get, ContentType.URL_ENCODED_FORM_DATA, {
                // calculationId: hcpCalculation
                playerId: playerId
            });

            return this.http.execute(request)
            .map((resp: Response) => {
                console.log("[Handicap] club : ", resp)
                let clubHandicap: Array<ClubHandicap> = resp.json();
                return clubHandicap;
            }).catch(this.http.handleError)
    }
    /**
     ...
     * @returns {Observable<R>}
     */
    // public saveNormalGameScorecard(scorecard: PlainScoreCard): Observable<PlainScoreCard> {

    //     let playedOnDate         = scorecard.playedOn;
    //     let playedOn             = moment(scorecard.playedOn).format("YYYY-MM-DD");
    //     let startTime            = scorecard.playerRoundScores.map(prs => prs.actualStartTime)
    //                                         .pop();
    ...
    //     let request = new RemoteRequest(global.ServerUrls.saveNormalGame,
    ...
    public getCourseHandicap(playerId?: number,teeBox?: string,firstNineCourse?: number,secondNineCourse?: number): Observable<number> {
        let hdrs = {};
        if(isPresent(playerId)) hdrs["Player-Id"] = playerId;
        console.log("get handicap history : ", playerId);
        console.log("get handicap history : ", hdrs);
        ...
        let request = new RemoteRequest(global.ServerUrls.getCourseHandicap,
            RequestMethod.Get, ContentType.URL_ENCODED_FORM_DATA, params);
            return this.http.execute(request)
            .map((resp: Response) => {
                console.log("Course Handicap ",resp)
                ...
                //     if(calcTime[1]) {
                //         h.calculatedOn = moment(calcTime[0],"YYYY-MM-DDTHH:mm:ssZ").toDate();
                //         h.gameRounds.forEach((gr: HandicapRound) => {
                //             let rDate = String(gr.roundDate).split("[");
                //             // console.log("[calctime] calcTime ", calcTime);
                //             gr.roundDate = moment(rDate[0],"YYYY-MM-DDTHH:mm:ssZ").toDate();
    ...
    public getHcpIdxBundles(playerCountry?: string): Observable<Array<PremiumFeatureBundle>> {
        let params = {};
        params['country'] = playerCountry;
        let request = new RemoteRequest(global.ServerUrls.getHcpIdxBundles,
            RequestMethod.Get, ContentType.URL_ENCODED_FORM_DATA, params);
            return this.http.execute(request)
            .map((resp: Response) => {
                console.log("Hcp Idx Bundles ",resp)
                let hcpBundles: any = resp.json();

                return hcpBundles;
            }).catch(this.http.handleError)
    }

    public getPremiumFeatureCollection(premiumFeaturePricing?: number): Observable<any> {
        let params = {};
        params['premium-feature-pricing'] = premiumFeaturePricing;
        console.log("Get Prem Feature Collection ",premiumFeaturePricing, params)
        let request = new RemoteRequest(global.ServerUrls.billplz_getPremiumFeatureCollection,
            RequestMethod.Get, ContentType.URL_ENCODED_FORM_DATA, params);
            return this.http.execute(request)
            .map((resp: Response) => {
                let premFeatPricing: any = resp.json();
                console.log("Get Prem Feature Collection ",premFeatPricing)
                return premFeatPricing;
            }).catch(this.http.handleError)
    }
