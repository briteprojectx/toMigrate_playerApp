...
import { adjustViewZIndex } from '../../../../globals';
@Component({
    templateUrl: 'club-user-management.html'
})
export class ClubUserManagementPage
{
    ...
    createContact() {
        ...
        console.log('create club user', _params)
        ...
        if (this.password !== this.password2) {
            let msg = "Please confirm your password with re-enter ";
            MessageDisplayUtil.displayErrorAlert(this.alertCtl, this.password + ' - ' + this.password2,
                msg, "OK");
            return;
        }
        ...
        let busy = this.loadCtl.create({
            content: "Creating club user..."
        });
        busy.present().then(() => {
            this.creating = true;
            // this.flightService.getBookingStats(). 
