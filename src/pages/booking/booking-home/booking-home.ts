...
@Component({
    templateUrl: 'booking-home.html',
    selector: 'booking-home-page'
})
export class BookingHomePage {
    ...
    cancelNextMonth: string;
    cancelPastDate: string;
    cancelCurrentDate: string;

    cancelStartDate: string;
    cancelEndDate: string;

    cancelPlayerBookingList: PagedData<TeeTimeBooking>;

    cancelCurrentPage: number;
    cancelTotalPages: number;
    cancelTotalItems: number;
    cancelPageSize: number;
    ...
    constructor(private nav: NavController,
        ...
        this.cancelNextMonth = moment().endOf('month').format("YYYY-MM-DD");
        this.cancelPastDate = moment().format("YYYY-MM-DD"); //moment().add(-1, 'month').format("YYYY-MM-DD");
        this.cancelStartDate = moment().add(-7,'days').format("YYYY-MM-DD");
        this.cancelEndDate = moment().add(7,'days').format("YYYY-MM-DD");
    ...
    ionViewDidLoad() {
        ...
        if(this.userType === 'player') {
            ...
            if(this.useHistorical) {
                ...
                this.refreshHistoryCancelBooking(0);
            } else {
    ...
    refreshBookingHome(refresh?: boolean) {
        if(refresh) {
            if(this.useHistorical) this.refreshHistoryBooking();
            else this.refreshPastBooking();

            if(this.useHistorical) this.refreshHistoryCancelBooking();
            else this.refreshCancelBooking();

            this.refreshInvitedBooking();
            this.refreshActiveBooking();
        }
        if(this.togglePastPanel) {
            if(this.useHistorical) this.refreshHistoryBooking();
            else this.refreshPastBooking();
        }
        else if(this.toggleCancelPanel) {
            if(this.useHistorical) this.refreshHistoryCancelBooking();
            else this.refreshCancelBooking();
        }
    ...
    refreshHistoryBooking(pageNo?: number) {
        ...
        let _pageSize = null;
        let _pageNo = null;
        let _descendingDate = true;
        if(pageNo) _pageNo = pageNo;
        this.flightService.searchBookingPlayer(_playerId,_clubId,_fromDate, _toDate,
            _statusType, _statusList, _courseId, _search, _pageSize, _pageNo,_descendingDate)
    ...
    refreshHistoryCancelBooking(pageNo?: number) {
        this.refresherCancel = true;
        this.cancelledBookingByPlayerList = [];
        this.loader = this.loadingCtrl.create({
            showBackdrop: false
        ...
        let _fromDate;
        let _toDate; 
        if(this.cancelMonthly) {
            _fromDate = moment(this.cancelPastDate).startOf('month').format("YYYY-MM-DD");
            // if(moment(this.cancelPastDate).format("YYYY-MM") === moment().format("YYYY-MM")) {
            //     _toDate = moment().add(-1,'days').format("YYYY-MM-DD");
            // } else 
            _toDate = moment(this.cancelPastDate).endOf('month').format("YYYY-MM-DD");
            
        } else if(!this.cancelMonthly) {
            _fromDate = this.cancelStartDate;
            _toDate = this.cancelEndDate;
        }
        
        this.activeDateSince = _fromDate;

        let _clubId = null;
        let _statusType = 'I'; //(A)ctive, (I)nactive, (B)oth
        let _statusList: Array<string> = [];
        let _courseId = null;
        let _search = '';
        let _pageSize = null;
        ...
        this.flightService.searchBookingPlayer(_playerId,_clubId,_fromDate, _toDate,
            _statusType, _statusList, _courseId, _search, _pageSize, _pageNo,_descendingDate)
        .subscribe((dataPlayerBookingList: PagedData<TeeTimeBooking>) => {
            this.cancelPlayerBookingList = dataPlayerBookingList;
            this.cancelCurrentPage = dataPlayerBookingList.currentPage;
            this.cancelTotalPages = dataPlayerBookingList.totalPages;
            this.cancelTotalItems = dataPlayerBookingList.totalItems;
            this.cancelPageSize = dataPlayerBookingList.totalInPage;
            let cancelledBookingByPlayerList: Array<TeeTimeBooking> = dataPlayerBookingList.items;
            this.loader.dismiss().then(()=>{
                this.refresherCancel = false;
                
                console.log("get past booking by player before : ", this.cancelledBookingByPlayerList)
                if(cancelledBookingByPlayerList.length > 0 ) {
                    this.cancelledBookingByPlayerList = cancelledBookingByPlayerList;

                    let _cancelBookings;
                    this.cancelledBookingByPlayerList.sort((a,b) => {
                        if(a.slotAssigned.teeOffDate == b.slotAssigned.teeOffDate)
                        {
                            return (a.slotAssigned.teeOffTime < b.slotAssigned.teeOffTime) ? 1 : (a.slotAssigned.teeOffTime > b.slotAssigned.teeOffTime) ? -1 : 0;
                        }
                    ...
                    // this.cancelledBookingByPlayerList = this.cancelledBookingByPlayerList.filter((t)=>{
                    //     console.log('filtered ', t.slotAssigned.teeOffDate, _today)
                    //     return moment(t.slotAssigned.teeOffDate,'YYYY-MM-DD').format('YYYY-MM-DD') <= _today;
                    // }).map((t)=>{
                    //     return t 
                    ...
                    //     console.log("get past booking date : ", t.slotAssigned.teeOffDate)
                    // })
                    console.log("get past booking by player after : ", _cancelBookings, this.cancelledBookingByPlayerList)
                }
            })
    ...
    nextDate(type?: string) {
        ...
        let _cancelMonth = moment(this.cancelPastDate).endOf('month').format('YYYY-MM-DD');
        ...
        if(type === 'cancel') this.cancelPastDate = moment(this.cancelPastDate).add(1, 'month').format("YYYY-MM-DD");
    ...
    prevDate(type?: string) {
        ...
        else if(type === 'cancel') this.cancelPastDate = moment(this.cancelPastDate).subtract(1, 'month').format("YYYY-MM-DD");
    ...
    nextPage(type?: string) {
        if(type === 'cancel') {
            let _cancelMonth = moment(this.cancelPastDate).endOf('month').format('YYYY-MM-DD');
            if (moment(_cancelMonth).isSameOrAfter(moment(this.cancelNextMonth).endOf('month'),'month')) return false;
            else return true;
            } else {
                let _pastMonth = moment(this.histPastDate).endOf('month').format('YYYY-MM-DD');
    ...
    getCancelPageNumber(count: number) {
        if(!this.cancelTotalPages) return;
        if(!this.cancelCurrentPage) return;
        return this.cancelCurrentPage + count;
    }
    ...
    prevCancelPage() {
        if(this.cancelCurrentPage === 1) return;
        let _currPage = this.cancelCurrentPage;
        this.refreshHistoryCancelBooking(_currPage - 1);
    }

    nextCancelPage() {
        if(this.cancelCurrentPage >= this.cancelTotalPages) return;
        let _currPage = this.cancelCurrentPage;
        this.refreshHistoryCancelBooking(_currPage + 1);
    }
}
