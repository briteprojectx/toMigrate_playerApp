...
import { LeagueLeaderboard, LeagueRoundDetails, LeagueRound } from '../../../data/mygolf.data';
/*
 ...
 */
@Component({
  templateUrl: 'competition-league-leaderboard.html',
  selector: 'competition-league-leaderboard-page'
})
export class LeagueLeaderboardPage {
  ...
  refreshLeagueLeaderboard(compId?: number) {
    let _compId;
    if(compId) _compId = compId;
    else _compId = this.competition.competitionId
    this.flightService.getLeagueLeaderboard(_compId)
    .subscribe((data: LeagueLeaderboard)=>{
      console.log("refresh league scorecards : ", data)
      if(data) {
        this.leagueLeaderboard = data;
  ...
  prevRound() {
    ...
    if(_idx>=0) {
      this.refreshLeagueLeaderboard(this.leagueRounds[_idx-1].competitionId)
