...
@Component({
    templateUrl: 'competition-flights.html',
    selector: 'competition-flights-page'
})
export class FlightsPage
{
    ...
    private refreshFlights(refresher) {
        let _currentFlightPublished = false;
        this.compService.getDetails(this.competition.competitionId)
        .subscribe((details: CompetitionDetails) => {
            if(details) {
                this.details = details;
                let _round: Array<GameRoundInfo>;
                _round = 
                this.details.gameRounds.filter((gameRound: GameRoundInfo)=>{
                    if(gameRound.roundNo === this.gameRound.roundNo) {
                        return true
                    }
                }).map((gameRound: GameRoundInfo)=>{
                    return gameRound
                });
                
                if(_round && _round.length > 0) {
                    _currentFlightPublished = _round[0].publishFlights
                }
                if(_currentFlightPublished) {
                  let loader = this.loadingCtl.create({
                  ...
                  if (loader)
                      ...
                      this.flightPublished = true;
              } else {
                  this.flightPublished = false;
                  this.nav.pop();
              }
    ...
    ionViewWillLeave() {
        this.nav.getPrevious().data.flightPublished = this.flightPublished;
        this.nav.getPrevious().data.fromFlight = true;
    }
